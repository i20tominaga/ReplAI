...
__export(src_exports, {
  ...
  comboboxSelectors: () => comboboxSelectors,
  comboboxStore: () => comboboxStore,
  createComboboxPlugin: () => createComboboxPlugin,
  getComboboxStoreById: () => getComboboxStoreById,
  getNextNonDisabledIndex: () => getNextNonDisabledIndex,
  getNextWrappingIndex: () => getNextWrappingIndex,
  getTextFromTrigger: () => getTextFromTrigger,
  onChangeCombobox: () => onChangeCombobox,
  onKeyDownCombobox: () => onKeyDownCombobox,
  useActiveComboboxStore: () => useActiveComboboxStore,
  useComboboxContent: () => useComboboxContent,
  useComboboxContentState: () => useComboboxContentState,
  useComboboxControls: () => useComboboxControls,
  useComboboxItem: () => useComboboxItem,
  useComboboxSelectors: () => useComboboxSelectors
});
module.exports = __toCommonJS(src_exports);

// src/combobox.store.ts
var import_plate_common = require("@udecode/plate-common");
var createComboboxStore = (state) => (0, import_plate_common.createZustandStore)(`combobox-${state.id}`)(state);
var comboboxStore = (0, import_plate_common.createZustandStore)("combobox")({
  activeId: null,
  byId: {},
  filteredItems: [],
  highlightedIndex: 0,
...
var getTextFromTrigger = (editor, {
  ...
  const escapedTrigger = (0, import_plate_common2.escapeRegExp)(trigger);
  ...
  while (true) {
    ...
    if (!start)
      break;
    start = (0, import_plate_common2.getPointBefore)(editor, start);
    const charRange = start && (0, import_plate_common2.getRange)(editor, start, end);
    const charText = (0, import_plate_common2.getEditorString)(editor, charRange);
    if (!charText.match(searchPattern)) {
      start = end;
      break;
    }
  }
  const range = start && (0, import_plate_common2.getRange)(editor, start, at);
...
var onChangeCombobox = (editor) => () => {
  ...
  for (const store of Object.values(byId)) {
    ...
    if (!selection || !(0, import_plate_common3.isCollapsed)(selection)) {
      continue;
    }
    ...
    const isCursorAfterTrigger = getTextFromTrigger(editor, {
      at: import_slate.Range.start(selection),
      searchPattern,
      trigger
    });
...
var onKeyDownCombobox = (editor) => (event) => {
  ...
  if ((0, import_plate_common4.isHotkey)("down", event)) {
  ...
  if ((0, import_plate_common4.isHotkey)("up", event)) {
    event.preventDefault();
    const newIndex = getNextWrappingIndex(
      -1,
      highlightedIndex,
  ...
  if (import_plate_common4.Hotkeys.isTab(editor, event) || (0, import_plate_common4.isHotkey)("enter", event)) {
    event.preventDefault();
...
var createComboboxPlugin = (0, import_plate_common5.createPluginFactory)({
  handlers: {
