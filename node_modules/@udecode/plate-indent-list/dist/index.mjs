...
var normalizeIndentListNotIndented = (editor, [node, path]) => {
  if (!isDefined2(node[KEY_INDENT6]) && (node[KEY_LIST_STYLE_TYPE] || node[KEY_LIST_START])) {
    unsetNodes5(editor, [KEY_LIST_STYLE_TYPE, KEY_LIST_START], { at: path });
    return true;
  }
};
...
var normalizeFirstIndentListStart = (editor, [node, path]) => {
  if (isDefined3(node[KEY_LIST_START])) {
    unsetNodes6(editor, KEY_LIST_START, { at: path });
    return true;
  }
};
...
var insertBreakIndentList = (editor) => {
  ...
  return function() {
    ...
    if (!isDefined5(node[KEY_LIST_STYLE_TYPE]) || node[KEY_LIST_STYLE_TYPE] !== KEY_TODO_STYLE_TYPE)
      return insertBreak();
    insertNodes(editor, {
...
var withIndentList = (editor, { options }) => {
  ...
  editor.apply = (operation) => {
    ...
    if (nodeBefore && operation.type === "set_node") {
      ...
      const prevIndent = operation.properties[KEY_INDENT7];
      const indent = operation.newProperties[KEY_INDENT7];
    ...
    deserializeHtml: {
      getNode: (element) => {
        ...
        return {
          ...
          type: getPluginType(editor, ELEMENT_DEFAULT2)
        };
      },
      isElement: true,
      rules: [
