...
var getEmptyCellNode = (editor, { header, newCellChildren = [editor.blockFactory()] }) => {
  return {
...
var getEmptyRowNode = (editor, _a = {}) => {
  var _b = _a, { colCount = 1 } = _b, options = __objRest(_b, ["colCount"]);
  return {
    children: Array.from({ length: colCount }).fill(colCount).map(() => getEmptyCellNode(editor, options)),
...
var getEmptyTableNode = (editor, {
  ...
  const rows = Array.from({ length: rowCount }).fill(rowCount).map(() => getEmptyRowNode(editor, { colCount, header, newCellChildren }));
  return {
    children: rows,
...
var getTableColumnCount = (tableNode) => {
  var _a, _b, _c, _d;
  return (_d = (_c = (_b = (_a = tableNode.children) == null ? void 0 : _a[0]) == null ? void 0 : _b.children) == null ? void 0 : _c.length) != null ? _d : 0;
};
...
var getTableMergeGridByRange = (editor, { at, format }) => {
  var _a;
  const { _cellIndices: cellIndices } = (0, import_server15.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  ...
  (_a = table.children) == null ? void 0 : _a.forEach((rowEl) => {
    ...
    const filteredChildren = (_a2 = rowElement.children) == null ? void 0 : _a2.filter((cellEl) => {
      ...
      return !!(cellElement == null ? void 0 : cellElement.children.length);
    });
...
var insertTable = (editor, {
  ...
  const pluginOptions = (0, import_server28.getPluginOptions)(editor, ELEMENT_TABLE);
    ...
    if (!(0, import_server28.someNode)(editor, {
      ...
      (0, import_server28.insertNodes)(
        ...
        getEmptyTableNode(editor, {
          colCount,
          header,
          newCellChildren: newCellChildren || (pluginOptions == null ? void 0 : pluginOptions.newCellChildren),
          rowCount
        }),
        __spreadValues({
          nextBlock: true
...
// src/merge/createEmptyCell.ts
var import_server29 = require("@udecode/plate-common/server");
var createEmptyCell = (editor, row, newCellChildren, header) => {
  const isHeaderRow = header === void 0 ? row.children.every(
    (c) => c.type === (0, import_server29.getPluginType)(editor, ELEMENT_TH)
  ) : header;
...
var insertTableMergeColumn = (editor, {
  ...
  const { _cellIndices: cellIndices } = (0, import_server30.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  ...
  if (!tableEntry)
    return;
  const { initialTableWidth, minColumnWidth, newCellChildren } = (0, import_server30.getPluginOptions)(editor, ELEMENT_TABLE);
  const [tableNode, tablePath] = tableEntry;
  ...
  affectedCells.forEach((cur) => {
    ...
    if (endCurI >= nextColIndex && !firstCol) {
      ...
      const row = (0, import_server30.getParentNode)(editor, currentCellPath);
      const rowElement = row[0];
      const emptyCell = __spreadProps(__spreadValues({}, createEmptyCell(editor, rowElement, newCellChildren, header)), {
        colSpan: 1,
        rowSpan: curRowSpan
      });
      (0, import_server30.insertElements)(editor, emptyCell, {
        at: placementPath
...
var insertTableColumn = (editor, options = {}) => {
  const { enableMerging } = (0, import_server31.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
  ...
  const cellEntry = fromCell ? (0, import_server31.findNode)(editor, {
    at: fromCell,
    match: { type: getCellTypes(editor) }
  ...
  const { initialTableWidth, minColumnWidth, newCellChildren } = (0, import_server31.getPluginOptions)(editor, ELEMENT_TABLE);
  (0, import_server31.withoutNormalizing)(editor, () => {
    ...
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === void 0 ? row.children.every(
        (c) => c.type === (0, import_server31.getPluginType)(editor, ELEMENT_TH)
      ) : header;
...
var insertTableMergeRow = (editor, {
  ...
  const { _cellIndices: cellIndices } = (0, import_server32.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  ...
  const { newCellChildren } = (0, import_server32.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  const cellEntry = (0, import_server32.findNode)(editor, {
    at: fromRow,
    match: { type: getCellTypes(editor) }
  });
  if (!cellEntry)
  ...
  affectedCells.forEach((cur) => {
    ...
    if (endCurI >= nextRowIndex && !firstRow) {
      (0, import_server32.setNodes)(
        editor,
        __spreadProps(__spreadValues({}, curCell), { rowSpan: curRowSpan + 1 }),
        { at: currentCellPath }
      );
    } else {
      const row = (0, import_server32.getParentNode)(editor, currentCellPath);
      const rowElement = row[0];
  ...
  (0, import_server32.withoutNormalizing)(editor, () => {
    (0, import_server32.insertElements)(
      editor,
      {
...
var insertTableRow = (editor, options = {}) => {
  const { enableMerging } = (0, import_server33.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
  ...
  const trEntry = fromRow ? (0, import_server33.findNode)(editor, {
    at: fromRow,
    match: { type: (0, import_server33.getPluginType)(editor, ELEMENT_TR) }
  }) : (0, import_server33.getBlockAbove)(editor, {
    match: { type: (0, import_server33.getPluginType)(editor, ELEMENT_TR) }
  });
  if (!trEntry)
    return;
  const [trNode, trPath] = trEntry;
  const tableEntry = (0, import_server33.getBlockAbove)(editor, {
    at: trPath,
    match: { type: (0, import_server33.getPluginType)(editor, ELEMENT_TABLE) }
  });
  if (!tableEntry)
    return;
  const { newCellChildren } = (0, import_server33.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === (0, import_server33.getPluginType)(editor, ELEMENT_TH)
      );
      return getEmptyCellNode(editor, __spreadValues({
        header: header != null ? header : isHeaderColumn
      }, newCellChildren));
    }),
    type: (0, import_server33.getPluginType)(editor, ELEMENT_TR)
  });
  (0, import_server33.withoutNormalizing)(editor, () => {
    (0, import_server33.insertElements)(editor, getEmptyRowNode2(), {
  ...
  if (!disableSelect) {
    ...
    (0, import_server33.select)(editor, nextCellPath);
  }
};
...
var moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if ((0, import_server34.hasNode)(editor, anchorPath) && (0, import_server34.hasNode)(editor, focusPath)) {
        (0, import_server34.select)(editor, {
          anchor: (0, import_server34.getStartPoint)(editor, anchorPath),
          focus: (0, import_server34.getStartPoint)(editor, focusPath)
        });
      }
      return true;
    }
  ...
  const cellEntry = (0, import_server34.getBlockAbove)(editor, {
    at,
    match: { type: getCellTypes(editor) }
  });
  if (cellEntry) {
    ...
    if ((0, import_server34.hasNode)(editor, nextCellPath)) {
      (0, import_server34.select)(editor, (0, import_server34.getStartPoint)(editor, nextCellPath));
    } else {
      const tablePath = cellPath.slice(0, -2);
      if (reverse) {
        (0, import_server34.withoutNormalizing)(editor, () => {
          (0, import_server34.select)(editor, (0, import_server34.getStartPoint)(editor, tablePath));
          (0, import_server34.moveSelection)(editor, { reverse: true });
        });
      } else {
        (0, import_server34.withoutNormalizing)(editor, () => {
          (0, import_server34.select)(editor, (0, import_server34.getEndPoint)(editor, tablePath));
          (0, import_server34.moveSelection)(editor);
        });
      }
    }
    return true;
...
var overrideSelectionFromCell = (editor, newSelection) => {
  ...
  if (!editor.currentKeyboardEvent || !["up", "down", "shift+up", "shift+right", "shift+down", "shift+left"].some(
    (key) => {
      const valid = (0, import_server35.isHotkey)(key, editor.currentKeyboardEvent);
      if (valid)
        hotkey = key;
      return valid;
    }
  ) || !((_a = editor.selection) == null ? void 0 : _a.focus) || !(newSelection == null ? void 0 : newSelection.focus) || !(0, import_server35.isRangeAcrossBlocks)(editor, {
    at: {
  ...
  if (edge && !(0, import_server35.isRangeInSameBlock)(editor, {
    at: editor.selection,
    match: { type: getCellTypes(editor) }
  })) {
    return;
...
var setBorderSize = (editor, size, {
  ...
  const cellEntry = (0, import_server36.findNode)(editor, {
    at,
    match: { type: getCellTypes(editor) }
  });
  if (!cellEntry)
  ...
  const setNodesOptions = {
    match: (n) => (0, import_server36.isElement)(n) && getCellTypes(editor).includes(n.type)
  };
  if (border === "top") {
  ...
  if (border === "all") {
    (0, import_server36.withoutNormalizing)(editor, () => {
...
var setTableColSize = (editor, { colIndex, width }, options = {}) => {
  ...
  (0, import_server37.setNodes)(editor, { colSizes }, { at: tablePath });
};
...
var setTableMarginLeft = (editor, { marginLeft }, options = {}) => {
  ...
  (0, import_server38.setNodes)(editor, { marginLeft }, { at: tablePath });
};
...
var setTableRowSize = (editor, { height, rowIndex }, options = {}) => {
  const table = (0, import_server39.findNode)(editor, __spreadValues({
    match: { type: ELEMENT_TABLE }
  }, options));
  if (!table)
    return;
  ...
  (0, import_server39.setNodes)(editor, { size: height }, { at: tableRowPath });
};
...
var onKeyDownTable = (editor, { type }) => (e) => {
  if (e.defaultPrevented)
    return;
  const isKeyDown = {
    "shift+down": (0, import_server40.isHotkey)("shift+down", e),
    "shift+left": (0, import_server40.isHotkey)("shift+left", e),
    "shift+right": (0, import_server40.isHotkey)("shift+right", e),
    "shift+up": (0, import_server40.isHotkey)("shift+up", e)
  };
  ...
  if (isTab || isUntab) {
    ...
    if (isUntab) {
      ...
      if (previousCell) {
        ...
        (0, import_server40.select)(editor, previousCellPath);
      }
    } else if (isTab) {
      const nextCell = getNextTableCell(editor, cell, cellPath, row);
      if (nextCell) {
        const [, nextCellPath] = nextCell;
        (0, import_server40.select)(editor, nextCellPath);
      }
    }
    e.preventDefault();
    e.stopPropagation();
  }
  if ((0, import_server40.isHotkey)("mod+a", e)) {
    const res = (0, import_server40.getAboveNode)(editor, { match: { type } });
    if (!res)
      return;
    const [, tablePath] = res;
    (0, import_server40.select)(editor, tablePath);
    e.preventDefault();
    e.stopPropagation();
  }
};
...
var preventDeleteTableCell = (editor, {
  ...
  const getPoint = reverse ? import_server41.getEndPoint : import_server41.getStartPoint;
  const getNextPoint = reverse ? import_server41.getPointAfter : import_server41.getPointBefore;
  if ((0, import_server41.isCollapsed)(selection)) {
    const cellEntry = (0, import_server41.getBlockAbove)(editor, {
      match: { type: getCellTypes(editor) }
    });
    if (cellEntry) {
      const [, cellPath] = cellEntry;
      ...
      const nextCellEntry = (0, import_server41.getBlockAbove)(editor, {
        at: nextPoint,
        match: { type: getCellTypes(editor) }
      });
      if (nextCellEntry) {
        (0, import_server41.moveSelection)(editor, { reverse: !reverse });
        return true;
      }
    }
  }
...
var withDeleteTable = (editor) => {
  ...
  editor.deleteFragment = (direction) => {
    if ((0, import_server41.isRangeInSameBlock)(editor, {
      match: (n) => n.type === (0, import_server41.getPluginType)(editor, ELEMENT_TABLE)
    })) {
      const cellEntries = getTableGridAbove(editor, { format: "cell" });
      if (cellEntries.length > 1) {
        (0, import_server41.withoutNormalizing)(editor, () => {
          cellEntries.forEach(([, cellPath]) => {
            (0, import_server41.replaceNodeChildren)(editor, {
              at: cellPath,
              nodes: editor.blockFactory()
            });
          });
          (0, import_server41.select)(editor, {
            anchor: (0, import_server41.getStartPoint)(editor, cellEntries[0][1]),
            focus: (0, import_server41.getEndPoint)(editor, cellEntries.at(-1)[1])
          });
        });
        return;
      }
...
var withGetFragmentTable = (editor) => {
  ...
  editor.getFragment = () => {
    ...
    fragment.forEach((node) => {
      if (node.type === (0, import_server42.getPluginType)(editor, ELEMENT_TABLE)) {
...
var withInsertFragmentTable = (editor, { options }) => {
  ...
  const myEditor = (0, import_server43.getTEditor)(editor);
  myEditor.insertFragment = (fragment) => {
    var _a, _b, _c;
    const insertedTable = fragment.find(
      (n) => n.type === (0, import_server43.getPluginType)(editor, ELEMENT_TABLE)
    );
    if (!insertedTable) {
      ...
      if (tableEntry) {
        ...
        if (cellEntries.length > 1) {
          cellEntries.forEach((cellEntry) => {
            if (cellEntry) {
              const [, cellPath] = cellEntry;
              (0, import_server43.replaceNodeChildren)(editor, {
                at: cellPath,
                nodes: (0, import_cloneDeep.default)(fragment)
              });
            }
          });
          (0, import_server43.select)(editor, {
            anchor: (0, import_server43.getStartPoint)(editor, cellEntries[0][1]),
            focus: (0, import_server43.getEndPoint)(editor, cellEntries.at(-1)[1])
          });
          return;
        }
      }
    ...
    if (insertedTable) {
      ...
      if (tableEntry) {
        ...
        if (cellEntry) {
          (0, import_server43.withoutNormalizing)(editor, () => {
            ...
            insertedRows.forEach((row) => {
              ...
              if (!initRow) {
                ...
                if (!(0, import_server43.hasNode)(editor, cellPath)) {
              ...
              insertedCells.forEach((cell) => {
                ...
                (0, import_server43.replaceNodeChildren)(editor, {
                  at: cellPath,
                  nodes: (0, import_cloneDeep.default)(cell.children)
                });
            ...
            if (lastCellPath) {
              (0, import_server43.select)(editor, {
                anchor: (0, import_server43.getStartPoint)(editor, startCellPath),
                focus: (0, import_server43.getEndPoint)(editor, lastCellPath)
              });
            }
          });
          return;
...
var withInsertTextTable = (editor, _plugin) => {
  ...
  editor.insertText = (text) => {
    var _a;
    if ((0, import_server44.isExpanded)(editor.selection)) {
      ...
      if (entry) {
        ...
        if (cellEntries.length > 1) {
          (0, import_server44.collapseSelection)(editor, {
            edge: "focus"
          });
        }
      }
...
var withMarkTable = (editor) => {
  ...
  editor.addMark = (key, value) => {
    const { selection } = editor;
    if (!selection || (0, import_server45.isCollapsed)(selection))
    ...
    matchesCell.forEach(([_cell, cellPath]) => {
      (0, import_server45.setNodes)(
        ...
        {
          at: cellPath,
          match: (n) => (0, import_server45.isText)(n),
          split: true,
          voids: true
        }
      );
  ...
  editor.removeMark = (key) => {
    ...
    if (matchesCell.length === 0)
      return removeMark(key);
    matchesCell.forEach(([_cell, cellPath]) => {
      (0, import_server45.unsetNodes)(editor, key, {
        at: cellPath,
        match: (n) => (0, import_server45.isText)(n),
        split: true,
        voids: true
  ...
  editor.getMarks = () => {
    ...
    const totalMarks = {};
    matchesCell.forEach(([_cell, cellPath]) => {
      const textNodeEntry = (0, import_server45.getNodeEntries)(editor, {
        at: cellPath,
        match: (n) => (0, import_server45.isText)(n)
      });
...
var withNormalizeTable = (editor) => {
  ...
  const myEditor = (0, import_server46.getTEditor)(editor);
  const { initialTableWidth } = (0, import_server46.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  myEditor.normalizeNode = ([node, path]) => {
    var _a, _b;
    if ((0, import_server46.isElement)(node)) {
      if (node.type === (0, import_server46.getPluginType)(editor, ELEMENT_TABLE)) {
        const tableEntry = (0, import_server46.getBlockAbove)(editor, {
          at: path,
          match: { type: (0, import_server46.getPluginType)(editor, ELEMENT_TABLE) }
        });
        if (tableEntry) {
          (0, import_server46.unwrapNodes)(editor, {
            at: path
          });
          return;
        }
        if (initialTableWidth) {
          ...
          if (colCount) {
            ...
            if (colSizes.length > 0) {
              (0, import_server46.setNodes)(editor, { colSizes }, { at: path });
              return;
            }
          }
        }
      }
      if (node.type === (0, import_server46.getPluginType)(editor, ELEMENT_TR)) {
        const parentEntry = (0, import_server46.getParentNode)(editor, path);
        if ((parentEntry == null ? void 0 : parentEntry[0].type) !== (0, import_server46.getPluginType)(editor, ELEMENT_TABLE)) {
          (0, import_server46.unwrapNodes)(editor, {
            at: path
          });
          return;
      ...
      if (getCellTypes(editor).includes(node.type)) {
        const { children } = node;
        const parentEntry = (0, import_server46.getParentNode)(editor, path);
        if ((parentEntry == null ? void 0 : parentEntry[0].type) !== (0, import_server46.getPluginType)(editor, ELEMENT_TR)) {
          (0, import_server46.unwrapNodes)(editor, {
            at: path
          });
          return;
        }
        if ((0, import_server46.isText)(children[0])) {
          (0, import_server46.wrapNodeChildren)(editor, editor.blockFactory({}, path), {
            at: path
          });
          return;
        }
...
var withSelectionTable = (editor) => {
  ...
  editor.apply = (op) => {
    if (op.type === "set_selection" && op.newProperties) {
      ...
      if (import_slate13.Range.isRange(newSelection) && (0, import_server47.isRangeAcrossBlocks)(editor, {
        at: newSelection,
        match: (n) => n.type === (0, import_server47.getPluginType)(editor, ELEMENT_TABLE)
      })) {
        const anchorEntry = (0, import_server47.getBlockAbove)(editor, {
        ...
        if (anchorEntry) {
          ...
          if (isBackward) {
            op.newProperties.focus = (0, import_server47.getStartPoint)(editor, anchorPath);
          } else {
            const pointBefore = (0, import_server47.getPointBefore)(editor, anchorPath);
            if (pointBefore) {
              op.newProperties.focus = (0, import_server47.getEndPoint)(editor, anchorPath);
            }
          }
        } else {
          const focusEntry = (0, import_server47.getBlockAbove)(editor, {
          ...
          if (focusEntry) {
            ...
            const isBackward = import_slate13.Range.isBackward(newSelection);
            if (isBackward) {
              const startPoint = (0, import_server47.getStartPoint)(editor, focusPath);
              const pointBefore = (0, import_server47.getPointBefore)(editor, startPoint);
              op.newProperties.focus = pointBefore != null ? pointBefore : startPoint;
            } else {
              op.newProperties.focus = (0, import_server47.getEndPoint)(editor, focusPath);
            }
          }
        }
      }
...
var withSetFragmentDataTable = (editor) => {
  ...
  editor.setFragmentData = (data, originEvent) => {
    ...
    (0, import_server48.withoutNormalizing)(editor, () => {
      tableRows.forEach((row, rowIndex) => {
        const rowCells = row.children;
        const rowPath = tablePath.concat(y + rowIndex);
        const cellStrings = [];
        const rowElement = row.type === (0, import_server48.getPluginType)(editor, ELEMENT_TH) ? document.createElement("th") : document.createElement("tr");
        rowCells.forEach((cell, cellIndex) => {
          data.clearData();
          const cellPath = rowPath.concat(x + cellIndex);
          (0, import_server48.select)(editor, {
            anchor: (0, import_server48.getStartPoint)(editor, cellPath),
            focus: (0, import_server48.getEndPoint)(editor, cellPath)
          });
          setFragmentData(data);
...
`;
      ...
      (0, import_server48.select)(editor, initialSelection);
...
var ELEMENT_TR = "tr";
var ELEMENT_TD = "td";
var createTablePlugin = (0, import_server49.createPluginFactory)({
  deserializeHtml: {
...
var useTableColSizes = (tableNode, { disableOverrides = false } = {}) => {
  ...
  const { enableUnsetSingleColSize } = (0, import_server50.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  ...
  import_react3.default.useEffect(() => {
    ...
    if (enableUnsetSingleColSize && colCount < 2 && ((_a = tableNode.colSizes) == null ? void 0 : _a.length)) {
      (0, import_server50.unsetNodes)(editor, "colSizes", {
...
var useTableCellElementResizableState = ({
  ...
  const { disableMarginLeft } = (0, import_server51.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  return {
...
var useTableCellElementResizable = ({
  ...
  const { minColumnWidth = 0 } = (0, import_server51.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  let initialWidth;
...
var useTableCellElementState = ({
  ...
  const { _cellIndices, enableMerging } = (0, import_server52.getPluginOptions)(
    editor,
    ELEMENT_TABLE
  );
  if (!enableMerging) {
...
var useTableElementState = ({
  ...
  const { disableMarginLeft, enableMerging, minColumnWidth } = (0, import_server53.getPluginOptions)(editor, ELEMENT_TABLE);
...
var mergeTableCells = (editor) => {
  (0, import_server54.withoutNormalizing)(editor, () => {
    var _a;
    const { _cellIndices } = (0, import_server54.getPluginOptions)(
      editor,
      ELEMENT_TABLE
    );
    const tableEntry = (0, import_server54.getBlockAbove)(editor, {
    ...
    for (const cellEntry of cellEntries) {
      const [el] = cellEntry;
      if (el.children.length !== 1 || !(0, import_server54.isElementEmpty)(editor, el.children[0])) {
        contents.push(...(0, import_cloneDeep2.default)(el.children));
      }
    }
    const cols = {};
    ...
    Object.values(cols).forEach((paths) => {
      paths == null ? void 0 : paths.forEach(() => {
        (0, import_server54.removeNodes)(editor, { at: paths[0] });
      });
    });
    const mergedCell = __spreadProps(__spreadValues({}, getEmptyCellNode(editor, {
      header: cellEntries[0][0].type === (0, import_server54.getPluginType)(editor, ELEMENT_TH),
      newCellChildren: contents
    })), {
      colSpan,
      rowSpan
    });
    (0, import_server54.insertElements)(editor, mergedCell, { at: cellEntries[0][1] });
    (0, import_server54.collapseSelection)(editor);
  });
};
...
var unmergeTableCells = (editor) => {
  (0, import_server55.withoutNormalizing)(editor, () => {
    const { _cellIndices: cellIndices } = (0, import_server55.getPluginOptions)(
      editor,
      ELEMENT_TABLE
    );
    const cellEntries = getTableGridAbove(editor, { format: "cell" });
    const [[cellElem, path]] = cellEntries;
    const createEmptyCell2 = (children) => {
      return __spreadProps(__spreadValues({}, getEmptyCellNode(editor, {
        header: cellElem.type === (0, import_server55.getPluginType)(editor, ELEMENT_TH),
    ...
    (0, import_server55.removeNodes)(editor, { at: path });
    const { col } = getCellIndices(
      cellIndices,
      cellElem
    );
    const getColPathForRow = (row) => {
      let newColPath = 0;
      const rowEntry = (0, import_server55.findNode)(editor, {
        at: [...tablePath, row],
        match: { type: (0, import_server55.getPluginType)(editor, ELEMENT_TR) }
      });
      if (!rowEntry) {
        return newColPath;
      }
    ...
    for (let i = 0; i < rowSpan; i++) {
      ...
      const rowEntry = (0, import_server55.findNode)(editor, {
        at: _rowPath,
        match: { type: (0, import_server55.getPluginType)(editor, ELEMENT_TABLE) }
      });
      for (let j = 0; j < colPaths.length; j++) {
        const cellToInsert = i === 0 && j === 0 ? createEmptyCell2(cellElem.children) : createEmptyCell2();
        if (rowEntry) {
          const currentColPath = i === 0 ? colPaths[j] : pathForNextRows;
          const pathForNewCell = [...tablePath, currentRowPath, currentColPath];
          (0, import_server55.insertElements)(editor, cellToInsert, { at: pathForNewCell });
        } else {
          newRowChildren.push(cellToInsert);
        }
      }
      if (!rowEntry) {
        (0, import_server55.insertElements)(
          editor,
          {
            children: newRowChildren,
            type: (0, import_server55.getPluginType)(editor, ELEMENT_TR)
          },
          { at: _rowPath }
        );
      }
...
var useTableMergeState = () => {
  ...
  const selectionExpanded = (0, import_plate_common15.useEditorSelector)(import_server56.isSelectionExpanded, []);
