...
var findCellByIndexes = (editor, table, searchRowIndex, searchColIndex) => {
  const { _cellIndices: cellIndices } = getPluginOptions2(
    editor,
    ELEMENT_TABLE
  );
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  const { enableMerging } = getPluginOptions4(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
...
var getCellPath = (editor, tableEntry, curRowIndex, curColIndex) => {
  const { _cellIndices: cellIndices } = getPluginOptions5(
    editor,
    ELEMENT_TABLE
  );
...
var deleteColumn = (editor) => {
  const { enableMerging } = getPluginOptions7(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
...
var deleteTableMergeRow = (editor) => {
  if (someNode3(editor, {
    ...
    const { _cellIndices: cellIndices } = getPluginOptions8(
      editor,
      ELEMENT_TABLE
    );
...
var deleteRow = (editor) => {
  const { enableMerging } = getPluginOptions9(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
...
var insertTable = (editor, {
  ...
  const pluginOptions = getPluginOptions10(editor, ELEMENT_TABLE);
...
var createEmptyCell = (editor, row, newCellChildren, header) => {
  const isHeaderRow = header === void 0 ? row.children.every(
    (c) => c.type === getPluginType13(editor, ELEMENT_TH)
  ) : header;
...
var insertTableMergeColumn = (editor, {
  ...
  if (!tableEntry)
    return;
  const { initialTableWidth, minColumnWidth, newCellChildren } = getPluginOptions11(editor, ELEMENT_TABLE);
  const [tableNode, tablePath] = tableEntry;
...
var insertTableColumn = (editor, options = {}) => {
  ...
  const { initialTableWidth, minColumnWidth, newCellChildren } = getPluginOptions12(editor, ELEMENT_TABLE);
  withoutNormalizing5(editor, () => {
    ...
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === void 0 ? row.children.every(
        (c) => c.type === getPluginType15(editor, ELEMENT_TH)
      ) : header;
      insertElements3(
        editor,
...
var insertTableMergeRow = (editor, {
  ...
  const { _cellIndices: cellIndices } = getPluginOptions13(
    editor,
    ELEMENT_TABLE
  );
  ...
  const { newCellChildren } = getPluginOptions13(
    editor,
    ELEMENT_TABLE
  );
...
var insertTableRow = (editor, options = {}) => {
  const { enableMerging } = getPluginOptions14(
    editor,
    ELEMENT_TABLE
  );
  if (enableMerging) {
  ...
  const trEntry = fromRow ? findNode9(editor, {
    at: fromRow,
    match: { type: getPluginType17(editor, ELEMENT_TR) }
  ...
  const tableEntry = getBlockAbove6(editor, {
    at: trPath,
    match: { type: getPluginType17(editor, ELEMENT_TABLE) }
  });
  if (!tableEntry)
    return;
  const { newCellChildren } = getPluginOptions14(
    editor,
    ELEMENT_TABLE
  );
  const getEmptyRowNode2 = () => ({
    children: trNode.children.map((_, i) => {
      ...
      const isHeaderColumn = !hasSingleRow && tableEntry[0].children.every(
        (n) => n.children[i].type === getPluginType17(editor, ELEMENT_TH)
      );
      return getEmptyCellNode(editor, __spreadValues({
        header: header != null ? header : isHeaderColumn
      }, newCellChildren));
    }),
    type: getPluginType17(editor, ELEMENT_TR)
  });
...
var withDeleteTable = (editor) => {
  ...
  editor.deleteFragment = (direction) => {
    if (isRangeInSameBlock2(editor, {
      match: (n) => n.type === getPluginType18(editor, ELEMENT_TABLE)
    })) {
...
var withGetFragmentTable = (editor) => {
  ...
  editor.getFragment = () => {
    ...
    fragment.forEach((node) => {
      if (node.type === getPluginType19(editor, ELEMENT_TABLE)) {
...
var withInsertFragmentTable = (editor, { options }) => {
  ...
  myEditor.insertFragment = (fragment) => {
    ...
    const insertedTable = fragment.find(
      (n) => n.type === getPluginType20(editor, ELEMENT_TABLE)
    );
    if (!insertedTable) {
...
var withNormalizeTable = (editor) => {
  ...
  myEditor.normalizeNode = ([node, path]) => {
    var _a, _b;
    if (isElement2(node)) {
      if (node.type === getPluginType21(editor, ELEMENT_TABLE)) {
        const tableEntry = getBlockAbove9(editor, {
          at: path,
          match: { type: getPluginType21(editor, ELEMENT_TABLE) }
        });
        if (tableEntry) {
          unwrapNodes(editor, {
            at: path
      ...
      if (node.type === getPluginType21(editor, ELEMENT_TR)) {
        const parentEntry = getParentNode4(editor, path);
        if ((parentEntry == null ? void 0 : parentEntry[0].type) !== getPluginType21(editor, ELEMENT_TABLE)) {
          unwrapNodes(editor, {
            at: path
          });
          return;
      ...
      if (getCellTypes(editor).includes(node.type)) {
        const { children } = node;
        const parentEntry = getParentNode4(editor, path);
        if ((parentEntry == null ? void 0 : parentEntry[0].type) !== getPluginType21(editor, ELEMENT_TR)) {
          unwrapNodes(editor, {
            at: path
          });
          return;
  ...
  editor.apply = (op) => {
    if (op.type === "set_selection" && op.newProperties) {
      ...
      if (Range2.isRange(newSelection) && isRangeAcrossBlocks2(editor, {
        at: newSelection,
        match: (n) => n.type === getPluginType22(editor, ELEMENT_TABLE)
      })) {
  ...
  editor.setFragmentData = (data, originEvent) => {
    ...
    withoutNormalizing12(editor, () => {
      tableRows.forEach((row, rowIndex) => {
        ...
        const cellStrings = [];
        const rowElement = row.type === getPluginType23(editor, ELEMENT_TH) ? document.createElement("th") : document.createElement("tr");
...
var mergeTableCells = (editor) => {
  withoutNormalizing13(editor, () => {
    var _a;
    const { _cellIndices } = getPluginOptions20(
      editor,
      ELEMENT_TABLE
    );
    ...
    for (const cellEntry of cellEntries) {
      const [el] = cellEntry;
      if (el.children.length !== 1 || !isElementEmpty(editor, el.children[0])) {
        contents.push(...cloneDeep2(el.children));
      }
    }
    const cols = {};
...
var unmergeTableCells = (editor) => {
  withoutNormalizing14(editor, () => {
    const { _cellIndices: cellIndices } = getPluginOptions21(
      editor,
      ELEMENT_TABLE
    );
    const cellEntries = getTableGridAbove(editor, { format: "cell" });
