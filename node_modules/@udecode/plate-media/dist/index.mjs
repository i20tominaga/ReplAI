...
var createAudioPlugin = createPluginFactory({
  isElement: true,
  isVoid: true,
  key: ELEMENT_AUDIO
});
...
var createFilePlugin = createPluginFactory2({
  isElement: true,
  isVoid: true,
  key: ELEMENT_FILE
});
...
var createImagePlugin = createPluginFactory3({
  isElement: true,
  isVoid: true,
  key: ELEMENT_IMAGE,
  then: (editor, { type }) => ({
...
import { isHotkey as isHotkey2, useEditorRef as useEditorRef2 } from "@udecode/plate-common";
...
var useImagePreview = ({
  ...
  const { zoomIn, zoomOut } = useZoom();
  const currentPreviewIndex = useMemo2(() => {
    if (!currentPreview)
      return null;
  ...
  const [prevDisabled, nextDisabled] = useMemo2(
    () => [
  ...
  const [zoomOutDisabled, zoomInDisabled] = useMemo2(
    () => [scale <= 0.5, scale >= 2],
    [scale]
  );
  useEffect3(() => {
    const keydown = (e) => {
      if (isHotkey2("escape")(e)) {
        e.stopPropagation();
        onClose();
      }
    };
...
var insertMedia = async (editor, _a = {}) => {
  var _b = _a, {
    getUrl,
    type = getPluginType2(editor, ELEMENT_IMAGE)
  ...
  if (!url)
    return;
  if (type === getPluginType2(editor, ELEMENT_IMAGE)) {
...
import { useElement as useElement2 } from "@udecode/plate-common";
...
var createMediaEmbedPlugin = createPluginFactory4({
  isElement: true,
  isVoid: true,
...
import { isUrl as isUrl2 } from "@udecode/plate-common/server";
...
import { createPluginFactory as createPluginFactory5 } from "@udecode/plate-common";
var ELEMENT_VIDEO = "video";
var createVideoPlugin = createPluginFactory5({
  isElement: true,
  isVoid: true,
  key: ELEMENT_VIDEO
});
...
import { createPrimitiveComponent as createPrimitiveComponent3, useElement as useElement3 } from "@udecode/plate-common";

// src/media/FloatingMedia/floatingMediaStore.ts
import { createZustandStore as createZustandStore2 } from "@udecode/plate-common/server";
...
var submitFloatingMedia = (editor, {
  ...
  const { isUrl: _isUrl = isUrl3, transformUrl } = getPluginOptions(editor, pluginKey);
...
var useMediaControllerState = () => {
  const [alignOpen, setAlignOpen] = React5.useState(false);
  return {
    alignOpen,
    setAlignOpen
  };
...
var createPlaceholderPlugin = createPluginFactory6({
  handlers: {
...
import { useEditorRef as useEditorRef5, useElement as useElement5 } from "@udecode/plate-common";
...
var usePlaceholderElementState = () => {
  ...
  const { mediaType } = useElement5(ELEMENT_PLACEHOLDER);
...
var usePlaceholderPopoverState = () => {
  ...
  const element = useElement6(ELEMENT_PLACEHOLDER);
...
var setMediaNode = (editor, props, options) => setNodes2(editor, props, options);
export {
  ELEMENT_AUDIO,
  ELEMENT_FILE,
  ELEMENT_IMAGE,
